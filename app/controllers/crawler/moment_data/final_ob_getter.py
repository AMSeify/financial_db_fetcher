...
from app.models.financial_db import FinalMoment, Company, OBHistory
...
async def set_rt_data():
    ...
    # Check the column types before conversion
    print(orderbook)
    ...
    if not results.empty:
        ...
        # Convert datetime to string format for both DataFrames
        date_format = "%Y-%m-%d %H:%M:%S"
        orderbook["datetime"] = pd.to_datetime(final["datetime"]).dt.strftime(date_format)
        orderbook["datetime"] = pd.to_datetime(results["datetime"]).dt.strftime(date_format)
